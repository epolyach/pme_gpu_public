# src/models/IsochroneTaperZH.jl
"""
    IsochroneTaperZH

Isochrone disc model with the Zang & Hohl (1978) taper applied to the
distribution function. The ZH taper uses a cubic polynomial in L/Jc to
smoothly suppress retrograde orbits below a critical angular momentum Jc.

Reference potential:  V(r) = -1 / (1 + sqrt(r^2 + 1))
"""
module IsochroneTaperZH
...
export IsochroneTaperZHModel, create_isochrone_taper_zh_model, setup_model

# ---------------------------------------------------------------------------
# Struct
# ---------------------------------------------------------------------------
struct IsochroneTaperZHModel <: AbstractGalacticModel end

# ---------------------------------------------------------------------------
# Factory
# ---------------------------------------------------------------------------
function create_isochrone_taper_zh_model(config::PMEConfig)
    mk = config.model.mk
    ...
    if Jc !== nothing
        return setup_model(IsochroneTaperZHModel, mk; unit_mass, unit_length, selfgravity, Jc, eta)
    elseif Rc !== nothing
        return setup_model(IsochroneTaperZHModel, mk; unit_mass, unit_length, selfgravity, Rc, eta)
    else
        R_max = 16.0
        return setup_model(IsochroneTaperZHModel, mk; unit_mass, unit_length, selfgravity, Rc=R_max, eta)
    end
end
...
# ===========================================================================
# Distribution function (ZH taper)
# ===========================================================================
...
function setup_model(::Type{IsochroneTaperZHModel}, mk::Int;
                     unit_mass::Real=1.0, unit_length::Real=1.0, selfgravity::Real=1.0,
                     Jc::Union{Real,Nothing}=nothing, Rc::Union{Real,Nothing}=nothing, eta::Real=1.0)
    ...
    if Rc !== nothing; params["Rc"] = Rc; end
    helpers = (
        model_name = "Kalnajs Isochrone - ZH Taper (Zang & Hohl 1978)",
    ...
    return ModelResults{Float64}(
        ...
        Sigma_d, sigma_u, helpers, "IsochroneTaperZH", params
    )
end

end # module IsochroneTaperZH
