"""
Multi-GPU Pi Elements calculation for PME using AMD GPUs.
Splits computation across multiple GPUs by distributing rows.
Now supports configurable precision (Float64 or Float32).
"""

module PiElementsMultiGPUAMD

using AMDGPU
using LinearAlgebra
using Printf

# Import get_k0 from Configuration (3 levels up)
...
"""
function calculate_pi_elements_multi_amd(config, 
                                       orbit_data, 
                                       z::Vector{Float64}, 
    ...
    for (i, device) in enumerate(gpu_devices)
        ...
        task = Threads.@spawn calculate_pi_rows_amd(config, orbit_data, z, psi_z, device, start_row_task, end_row_task)
        push!(tasks, task)
    end
    ...
    for (i, task) in enumerate(tasks)
        try
            ...
            start_row, end_row = gpu_row_ranges[i]
            @error "GPU $device failed computing rows $start_row:$end_row" exception=(e, catch_backtrace())
            rethrow(e)
        end
    end
...
"""
function calculate_pi_rows_amd(config, 
                                orbit_data, 
                                z::Vector{Float64}, 
                                psi_z::Vector{Float64}, 
                                device_id::Int,
    ...
    # Set GPU device (AMDGPU requires HIPDevice, not integer)
    ...
    total_memory = AMDGPU.HIP.properties(AMDGPU.device()).totalGlobalMem / (1024^3)
    ...
    z_gpu = ROCArray(T.(z))
    pi4_chunk_gpu = ROCArray(pi4_chunk)
    ...
    @roc groupsize=threads_per_block gridsize=blocks_per_grid pi_element_chunk_kernel!(
    ...
    # Free GPU memory
    AMDGPU.unsafe_free!(pi4_chunk_gpu)
