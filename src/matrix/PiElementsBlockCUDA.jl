...
"""
Compute Pi elements for a single (l_i, l_j) resonance pair on CUDA GPU.
...
"""
function compute_pi_block_cuda(config, orbit_data, z::Vector{Float64}, psi_z::Vector{Float64}, 
    ...
    z_gpu = CuArray(T.(z))
    pi4_block_gpu = CuArray(pi4_block)
    ...
    @cuda threads=threads_per_block blocks=blocks_per_grid pi_block_kernel!(
    ...
    # Free GPU memory
    CUDA.unsafe_free!(pi4_block_gpu)
...
end # module PiElementsBlockCUDA
