...
Plain (sharp-edge) Isochrone disc model with no taper applied to the distribution
function. The DF is unidirectional (L >= 0 only), which produces a sharp edge in
angular momentum space. This model therefore requires the **generalized matrix
method** for a correct stability analysis.
...
export IsochroneModel, create_isochrone_model, setup_model
...
# Factory
# ---------------------------------------------------------------------------
function create_isochrone_model(config::PMEConfig)
    mk = config.model.mk
...
function setup_model(::Type{IsochroneModel}, mk::Int;
    ...
    helpers = (
        model_name = "Kalnajs Isochrone - Plain (no taper, sharp edge)",
    ...
    return ModelResults{Float64}(
        ...
        Sigma_d, sigma_u, helpers, "Isochrone", params
    )
end
